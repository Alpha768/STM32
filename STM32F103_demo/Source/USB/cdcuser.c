...
BOOL CDC_SetControlLineState (unsigned short wControlSignalBitmap) {
  ...
  GPIOA->CRL &= ~0x000000F0;                /* clear port PA1 */
  GPIOA->CRL |=  0x00000030;                /* PA1 General purpose output push-pull, max speed 50 MHz */
  if(wControlSignalBitmap&0x01)
  {
  GPIOA->BSRR = 0x0002;                         // set PA1 
  }
  else
  {
  GPIOA->BRR = 0x0002;                          // reset PA1 
  }
  return (__TRUE);
}
