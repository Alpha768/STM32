...
void USB_EndPoint1 (U32 event) {
   ...
   switch (event) {
    case USB_EVT_IN:
		...
		if(numBytes)
		{
		QueueRead(&USBTransmitBuffer,buffer,numBytes);
		USB_WriteEP (0x81, buffer, numBytes);
		}
    ...
    case USB_EVT_OUT:
		...
		numBytes=USB_ReadEP(0x01, buffer);
		if(ExeUsbFilterCMD(0,buffer,numBytes))
			return;
		if(QueueFreeCount(&USBReceiveBuffer)>=numBytes)
		{
			QueueWrite(&USBReceiveBuffer,buffer,numBytes);
		}
...
void USB_EndPoint2 (U32 event) {
     switch (event) {
    case USB_EVT_IN:
      //Data_BulkIn ();                 /* data expected from Host */
      break;
    case USB_EVT_OUT:
      //Data_BulkOut ();                /* data received from Host */
      break;
  }
}
...
void USB_EndPoint3 (U32 event) {
    switch (event) {
    case USB_EVT_IN:
      //DeviceCtrl_BulkIn ();                 /* data expected from Host */
      break;
    case USB_EVT_OUT:
      //DeviceCtrl_BulkOut ();                /* data received from Host */
      break;
  }
}
